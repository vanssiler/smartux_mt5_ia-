import pandas as pd
import numpy as np  # ✅ Import necessário
...
class SmartuxAgent:
    ...
    def analyze(self, symbol, df=None):
        ...
        if hasattr(self.ml_controller.model, "predict_proba"):
            try:
                X_proba = df_pred.tail(1)
                X_proba.columns = self.ml_controller.feature_names

                if X_proba.empty or X_proba.shape[0] < 1:
                    raise ValueError("predict_proba: entrada vazia.")

                proba_array = self.ml_controller.model.predict_proba(X_proba)
                print(f"[DEBUG] predict_proba shape: {proba_array.shape}")
                print(f"[DEBUG] predict_proba array: {proba_array}")

                if isinstance(proba_array, np.ndarray) and prediction < proba_array.shape[1]:
                    confidence_value = proba_array[0][prediction]
                    confidence = f"{confidence_value * 100:.2f}%"
                else:
                    raise ValueError("predict_proba retornou dados inválidos.")
            except Exception as e:
                print(f"[IA] Erro ao calcular confiança com predict_proba: {e}")
