...
class SmartuxAgent:
    ...
    def analyze(self, symbol, df=None):
        ...
        if hasattr(self.ml_controller.model, "predict_proba"):
            try:
                proba_values = self.ml_controller.model.predict_proba(df_pred)
    ...
    def auto_trade_decision(self, symbol, df):
        ...
        # Pega apenas a última linha para decisão
        ...
        # SL/TP dinâmicos com base no ATR (volatilidade média)
        atr_value = df_recent["atr"].iloc[-1] if "atr" in df_recent.columns else 0.001
        atr_value = max(atr_value, 0.0001)  # Evita 0
